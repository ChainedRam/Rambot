...
namespace TeamSpeak3QueryApi.Net.Specialized
{
    public class TeamSpeakClient
    {
        ...
        public string Subscribe2<T>(Action<IReadOnlyCollection<T>> callback)
            where T : Notification
        {
            var notification = GetNotificationType<T>();
            ...
            _callbacks.Add(Tuple.Create(notification, callback as object, cb));
            Client.Subscribe("notifytextmessage", cb);
        ...
        public Task SendServerMessage(string message, int serverId)
        {
            return SendMessage(message, MessageTarget.Server, serverId);
        }
        public Task SendChannelMessage(string message, int targetChannel)
        {
            return SendMessage(message, MessageTarget.Channel, targetChannel);
        }
        public Task SendPrivateMessage(string message, int clientId)
        {
            return SendMessage(message, MessageTarget.Private, clientId);
        }
